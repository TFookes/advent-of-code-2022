...
if __name__ == "__main__":
    with open("./inputs.txt", "r") as inputs:
        ...
        free_space = 70000000 - dict(disk.nodes(data="size", default=0))["/.0"]
        print(free_space)
        directory_options = []
        for node, attributes in disk.nodes(data=True):
            if attributes['directory'] == True and attributes["size"] >= 30000000 - free_space:
                print(node, attributes)
                directory_options.append(attributes["size"])

        print(min(directory_options))
